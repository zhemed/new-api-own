...
| GitHub 私有仓库 | 克隆/推送需带 token：`git clone https://zhemed:<token>@github.com/zhemed/new-api-own.git`（token 在 `（本机凭据文件）`；git 操作需 `export HOME=/root`） |
...
4. 提交后推送：`export HOME=/root && git push origin main`（私有仓库，凭据在 `（本机凭据文件）`）
