# 调查 x-opencode-session / （实例兜底值已脱敏） 渠道配置

## 结论（TL;DR）

这不是可疑改动，而是 **2026-09-12 有意配置**、并已写进 `MAINTENANCE.md` 的机制：线上唯一渠道（`（渠道名已脱敏）` →
`https://opencode.ai/zen/go`，即 OpenCode Go / Zen）上有一条 `param_override`，作用是**透传客户端自带的会话头，
客户端没带时兜底填 `（实例兜底值已脱敏）`**。上游因此看到我们的出口 IP + 一个非 OpenCode 客户端自带的会话标识。

## 证据（可复现）

### 1. 线上配置原文（取自 （实例域名已脱敏） 数据备份，只读）

来源：`（非公开目录）/（实例域名已脱敏）/20260916-165855/data/one-api.db.hot-20260916-165855` → `channels` 表。

```
id=1  name=（渠道名已脱敏）  type=60(ChannelTypeNewAPI)  status=1  base_url=https://opencode.ai/zen/go
header_override = (空)
param_override  = {"operations":[
  {"mode":"pass_headers","value":["x-opencode-session","Session-Id","X-Session-Id"],"keep_origin":true},
  {"mode":"set_header","path":"x-opencode-session","value":"（实例兜底值已脱敏）","keep_origin":true}
]}
```

- 全库扫描 `（实例兜底值已脱敏）`：**只出现 1 次**，就在 `channels.param_override`；没有出现在任何其它表/字段。
- 该库只有 1 个渠道（启用 1 个）。
...
| 2026-09-12 10:41 备份（planB-working） | 已带上 `（实例兜底值已脱敏）` 兜底配置 → **配置在这一夜之间加上** |
...
- 前端预设：`web/src/features/channels/components/dialogs/param-override-editor-dialog.tsx:355+`（示例兜底值是
  `opencode-go-fallback`；线上实际用 `（实例兜底值已脱敏）`）。
- 文档：`MAINTENANCE.md:212-269`（含上游实测结论与探针诊断手法）。

## 上游能看到什么

- 我们的出口 IP；
- `x-opencode-session: （实例兜底值已脱敏）`（**仅当客户端未自带**该头；自带则原样透传）；
...
1. **共用一个缓存桶**：所有不自带会话头的客户端都用 `（实例兜底值已脱敏）`，对话级 prompt-cache 亲和丢失；反过来，不同对话/用户可能命中同一份上游缓存条目（成本更低，但存在跨对话缓存共享）。`MAINTENANCE.md:240` 已写明这个代价。
2. **`-01` 的编号没用起来**：命名暗示可按客户端/用户分桶，目前只有单一兜底值，没有分桶。
3. 该头不是鉴权，伪造它不构成对上游的鉴权绕过；但它是上游做路由/缓存决策的依据。

## 盲区（不许假装查全）

- 证据来自 **2026-09-16 的数据备份**；本机没有到 `（内网地址已脱敏）` 的 SSH 私钥（`Permission denied (publickey,password)`），无法直连线上库核对"此刻"的配置。
