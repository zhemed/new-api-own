...
`new-api-own` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/new-api-own/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/new-api-own/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee new-api-own` to
...
python3 ./.trellis/scripts/task.py archive 00-join-new-api-own
```
